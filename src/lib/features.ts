...
export function buildFeatureArray(history: DataRow[]): number[][] {
  ...
  for (let i = 0; i < history.length; i++) {
    ...
    const lag1 = i > 0 ? (history[i - 1].persentaseCakupan ?? 0) : 0
    const lag2 = i > 1 ? (history[i - 2].persentaseCakupan ?? 0) : 0
    const lag3 = i > 2 ? (history[i - 3].persentaseCakupan ?? 0) : 0

    result.push([
      row.jumlahBayi6Bulan,
